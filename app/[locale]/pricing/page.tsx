...
export async function generateMetadata(
  ...
  return buildMetadata(locale, 'pricing_title', 'pricing_desc', '/pricing')
}
...
export default function PricingPage() {
  ...
  const features = [
    t('feat_unlimited'), t('feat_citations'), t('feat_wayback'), t('feat_zotero'), t('feat_languages')
  ]
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="mx-auto max-w-3xl px-6 pb-16 pt-16 text-center">
        <Badge variant="outline" className="mb-6 border-green-300 bg-green-50 text-green-700">
          {t('badge')}
        </Badge>
      ...
      <section className="mx-auto max-w-sm px-6 pb-20">
        <Card className="border-navy bg-navy shadow-xl shadow-navy/20">
          <CardHeader className="pb-4">
            <p className="text-xs font-bold uppercase tracking-widest text-white/50">{t('free_label')}</p>
            <div className="mt-2 flex items-baseline gap-1">
              <span className="text-4xl font-black text-white">{t('free_price')}</span>
              <span className="text-white/50">{t('per_month')}</span>
            </div>
            <p className="mt-1 text-sm text-white/60">{t('free_sub')}</p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Link href="/login" className={cn(buttonVariants(), 'mt-4 w-full bg-yellow-400 font-semibold text-navy hover:bg-yellow-300')}>
              {t('free_cta')}
            </Link>
          </CardContent>
        </Card>
